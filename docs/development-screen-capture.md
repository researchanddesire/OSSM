...
Development profiles `development-4mb`, `development-16mb` enable the
...
```sh
pio run -d Software -e development-4mb -t buildprog
```
...
python scripts/capture_screen.py --port COM_PORT --product OSSM --output-dir captures --name current
```
